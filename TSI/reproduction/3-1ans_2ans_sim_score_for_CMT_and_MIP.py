...
def parse_option():
    ...
    parser.add_argument('--baseline', default='CMT-1que-2que-1.0-extract', type=str, help='')
...
def main(args):
    ...
    model = T5ForConditionalGeneration.from_pretrained('google-t5/t5-3b', dtype=torch.float16)
    model.cuda()
    for file in dataset_to_fileList[args.dataset]:
        ...
        for sample in samples:
            i+=1
            sentence,label,target_position,target_word,pos_tag,gloss,eg_sent,prompt,answer_text,answer_kdg_text, answer1_list, answer2_list = sample
            print(i)
            ...
            # contextual meaning
            answer2_scores_list = []
            for answer2_1 in answer2_list:
                answer2_scores = []
                input_list = []
                for answer2_2 in answer2_list:
                    input_list.append("stsb sentence 1: "+answer2_1+" sentence 2: "+answer2_2)
                input_ids = tokenizer(input_list, return_tensors="pt", padding=True).input_ids 
                input_ids = input_ids.cuda()
                stsb_ids = model.generate(input_ids, max_new_tokens=1000)
                for decode_i in range(len(stsb_ids)):
                    stsb = tokenizer.decode(stsb_ids[decode_i],skip_special_tokens=True)
                    if answer2_list[decode_i] == answer2_1:
                        answer2_scores.append(5.0)
                    else:
                        if stsb == 'entailment':
                            stsb = '5.0'
                        try: 
                ...
                answer2_scores_list.append(answer2_scores)

            new_samples.append(sample + [str(answer1_scores_list)] + [str(answer2_scores_list)])
        head = ['sentence','label','target_position','target_word','pos_tag','gloss','eg_sent','questions', 'answers', 'answers_kdg', 'answer1_extract', 'answer2_extract', 'answer1_score', 'answer2_score']
        dataloader.write_dataset(file, head, new_samples)    
        with open('results/results.txt', 'a') as f:
            f.write(f"Start 3: {now}")
            f.write(f"End 3: {datetime.now()}")  
