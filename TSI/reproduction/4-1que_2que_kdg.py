...
def parse_option():
    ...
    parser.add_argument('--data_dir', default='results-similar-score/CMT-1que-2que-1.0-extract/qwen3/', type=str)
    parser.add_argument('--baseline', default='CMT-1que-2que-add-kdg', type=str, help='')
...
def main(args):
    ...
    for file in dataset_to_fileList[args.dataset]:
        ...
        with open('results/results.txt', 'a') as f:
            f.write(f"Start 4: {now}\n")
            f.write(f"End 4: {datetime.now()}\n") 
        with open('results/results.txt', 'a') as f:
            f.write(f"count_source_need_knowledge: {count_source_need_knowledge}\n")
            f.write(f"count_target_need_knowledge: {count_target_need_knowledge}\n")
