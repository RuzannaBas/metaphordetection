...
def parse_option():
    ...
    parser.add_argument('--data_dir', default='results/CMT-3que-1.0-vrto0.8-sim2.0-5-3/qwen3', type=str)
...
def main(args):
    ...
    for file in dataset_to_fileList[args.dataset]:
        ...
        for sample_id in range(len(samples)):
            ...
            if (final_answer3 == 'exist' and label == '1') or (final_answer3 == 'non-exist' and label == '0'):
                correct +=1
        ...
        with open(f'{args.dir}results/results.txt', 'a') as f:
            f.write(f"Start 6: {now}\n")
            f.write(f"End 6: {datetime.now()}\n") 
        with open(f'{args.dir}results/results.txt', 'a') as f:
            f.write(f"Correct: {correct}\n")
            f.write(f"Metaphor {metaphor}\n")
            f.write(f"Literal {literal}\n")
            f.write(report)
            f.write('\n')
