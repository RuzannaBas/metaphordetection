...
def parse_option():
    ...
    parser.add_argument('--data_dir', default='results/CMT-1que-2que-add-kdg-1.0-vrto0.8-sim2.0-5-3/qwen3/', type=str)
    ...
    for file in dataset_to_fileList[args.dataset]:
        ...
        for sample_id in range(len(new_samples), len(samples)):
            ...
            if second_final_answer == '':
                second_final_answer = random.sample(answers[1], 1)

            prompt_contrast = '''Based on the above information, decide whether the source domain and target domain belong to fundamentally different conceptual categories. If one domain is contained within, is a subtype or, or is a specific example of the other domain, it is not different.'''
            ...
            answers_res = []
            for _ in range(args.add_kdg_number):
                response = chat(
                    model=_model,
                    messages=dialogs,
                    think=args.think,
                ...
                answers_res.append(response['message']['content'])
        ...
        with open('results/results.txt', 'a') as f:
            f.write(f"Start 5: {now}\n")
            f.write(f"End 5: {datetime.now()}\n") 
